...
def make_classical_features(gfz, index):
    feat = pd.DataFrame(index=index)
    feat["SN"] = gfz["SN"].reindex(index, method="ffill")
    feat["F107"] = gfz["F107obs"].reindex(index, method="ffill")
    feat["Ap_t"] = gfz["Ap"].reindex(index, method="ffill")
    feat["Ap_3d_mean"] = gfz["Ap"].rolling(3).mean().reindex(index, method="ffill")
    feat["Ap_7d_mean"] = gfz["Ap"].rolling(7).mean().reindex(index, method="ffill")
    return feat
...
def main():
    emb = load_embeddings()
    gfz = fetch_gfz()
    classical = make_classical_features(gfz, emb.index)
    ...
    header = f"{'lead':>5} | {'persistence':>11} | {'classical':>9} | {'embed':>9} (n_pca,alpha) | {'embed+classical':>16} (n_pca,alpha) | {'embed(MLP)':>11}"
    print(header)
    print("-" * 120)

    for lead in LEAD_TIMES:
        target_dates = emb.index + pd.Timedelta(days=lead)
        target = gfz["Ap"].reindex(target_dates)
        ...
        classical_rmse = fit_rmse(
            LinearRegression(), train[cls_cols], y_train, test[cls_cols], y_test,
        )
        ...
        embed_rmse = fit_rmse(embed_best, train[emb_cols], y_train, test[emb_cols], y_test)
        ...
        combined_rmse = fit_rmse(combined_best, X_train_combined, y_train, X_test_combined, y_test)
        ...
        mlp_rmse = fit_rmse(mlp_best, train[emb_cols], y_train, test[emb_cols], y_test)
        ...
        print(f"{lead:>5} | {pers_rmse:>11.3f} | {classical_rmse:>9.3f} | {embed_rmse:>9.3f} {e_tag:>12} | {combined_rmse:>16.3f} {c_tag:>12} | {mlp_rmse:>11.3f} {m_tag}")
